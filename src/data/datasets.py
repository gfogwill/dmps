...
def default_transformer(dsdict, **kwargs):
    ...
    logger.error(
        f"'{transformer_name}()' function not found. Define it add it to the `user` namespace for correct behavior")
    return dsdict
        ...
        if self.catalog_dir_fq.exists():
            ...
            if catalog_data is not None and merge_priority is None and overwrite is False:
                raise AttributeError(
                    "Must specity `merge_priority` or `overwrite=True` if using `catalog_data` with an existing Catalog")
...
class Dataset:
    ...
    def update_catalog(self, catalog_path=None, catalog_file='datasets.json'):
        ...
        catalog, catalog_file_fq = dataset_catalog(catalog_path=catalog_path, catalog_file=catalog_file,
                                                   include_filename=True, keys_only=False)
    ...
    def from_catalog(cls, dataset_name,
        ...
        if catalog_hashes is not None:
            if not ds.verify_hashes(catalog_hashes):
                raise Exception(
                    f"Dataset '{dataset_name}' hashes {generated_hashes} do not match catalog: {catalog_hashes}")
...
class DataSource(object):
    ...
    def add_metadata(self, filename=None, contents=None, metadata_path=None, kind='DESCR', unpack_action='copy',
                     force=False):
    ...
    def add_file(self, source_file=None, *, hash_type='sha1', hash_value=None,
        ...
        logger.warning(
            "Reproducibility Issue: add_file is often not reproducible. If possible, use add_manual_download instead")
        self.file_dict[file_name] = fetch_dict
        self.fetched_ = False
        ...
        if self.fetched_ and force_download is False:
            ...
            for filename, item in self.file_dict.items():
                ...
                if raw_file_hash != item['hash_value']:
                    logger.warning(
                        f"{raw_data_file.name} hash invalid ({raw_file_hash} != {item['hash_value']}). Invalidating fetch cache.")
                    self.fetched_ = False
                    break
            else:
...
class TransformerGraph:
    ...
    * produces one or more `Dataset` objects (the `output_datasets`).
    ...
    def __init__(self, catalog_path=None, transformer_file='transformers.json', dataset_file='datasets.json'):
        ...
        self.transformers, self._transformer_catalog_fq = transformer_catalog(catalog_path=catalog_path,
                                                                              catalog_file=transformer_file,
                                                                              include_filename=True)
        self.datasets, self._dataset_catalog_fq = dataset_catalog(catalog_path=catalog_path, catalog_file=dataset_file,
                                                                  include_filename=True, keys_only=False)
        ...
        while queue:
            ...
            if vertex not in visited:
                ...
                if satisfied:
                    if force:
                        logger.debug(
                            f"All dependencies {parents} satisfied for edge: '{edge}' but force=True specified.")
                    else:
    ...
    def process_edge(self, edge_name, write_catalog=True, force=False, dataset_path=None):
        ...
        for in_ds in edge.get('input_datasets', []):  # sources have no inputs
            ...
            if in_ds not in self.datasets:
                raise Exception(
                    f"Edge '{edge_name}' specifies an input dataset, '{in_ds}' that is not in the dataset catalog")
            ...
            if not self.check_dataset_hashes(in_ds, ds.HASHES):
                if force and write_catalog:
                    ...
                    raise Exception(
                        f"Cached Dataset '{in_ds}' hashes {self.datasets[in_ds]['hashes']} do not match catalog {ds.HASHES}")
            dsdict[in_ds] = ds
